    ...
    <div class="py-6 max-w-xl mx-auto">
        <div class="bg-white border border-slate-100 rounded-3xl p-6 md:p-8 shadow-sm">
            ...
            <form action="{{ route('merchant.shop.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <!-- City ID -->
                <div>
                    <label for="city_id" class="block text-sm font-semibold text-slate-700 mb-2">Pilih Kota Asal (ID Kota RajaOngkir)</label>
                    <input type="number" name="city_id" id="city_id" value="{{ old('city_id', $shop->city_id) }}" required class="w-full border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 rounded-xl py-2.5 text-sm">
                    @error('city_id')
                        <span class="text-xs text-rose-500 block mt-1">{{ $message }}</span>
                    @enderror
                </div>
