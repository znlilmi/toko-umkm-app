        ...
        <div class="bg-white border border-slate-100 rounded-3xl p-6 md:p-8 shadow-sm">
            ...
            <form action="{{ route('merchant.products.update', $product->id) }}" method="POST" x-data="productForm()" @submit="submitForm($event)" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" class="w-full border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 rounded-xl py-2.5 text-sm">{{ old('description', $product->description) }}</textarea>
                    @error('description')
                        <span class="text-xs text-rose-500 block mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="weight" id="weight" value="{{ old('weight', $product->weight) }}" required min="1" class="w-full border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 rounded-xl py-2.5 text-sm">
                    @error('weight')
                        <span class="text-xs text-rose-500 block mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Pilih Kategori Produk</label>
                    <div class="grid grid-cols-2 gap-3 max-h-48 overflow-y-auto border border-slate-200 rounded-xl p-3 bg-white">
                        ...
                        @foreach($categories as $category)
                            <label class="flex items-center space-x-2 p-1.5 hover:bg-slate-50 rounded-lg cursor-pointer text-sm">
                                <input type="checkbox" name="category_ids[]" value="{{ $category->id }}" {{ in_array($category->id, $selectedCategoryIds) ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $product->is_active) ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
